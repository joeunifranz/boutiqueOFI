...
class reservationController extends mainModel{
    ...
    private function cargarConfigBnbQr(){
        if(!defined('BNB_API_BASE_URL')){
            $ruta = __DIR__."/../../config/bnb_qr.php";
            if(file_exists($ruta)){
                require_once $ruta;
            }
        }
    ...
    private function obtenerUltimoQrGeneradoPorCodigo($codigo){
        ...
        try{
            // Preferimos mostrar el último QR generado (BNB/BISA)
            $stmt = $this->conectar()->prepare("SELECT * FROM reserva_pago WHERE reserva_codigo=:c AND pago_proveedor IN ('bnb','bisa') ORDER BY reserva_pago_id DESC LIMIT 1");
    ...
    private function registrarReservaPagoCreadoBnb($codigo,$qrId,$qrString,$monto,$moneda,$raw=null){
        if(!$this->tablaReservaPagoExiste()){
            return false;
        }
        try{
            $now = date('Y-m-d H:i:s');
            $stmt = $this->conectar()->prepare("INSERT INTO reserva_pago (reserva_codigo,pago_proveedor,pago_payment_id,pago_status,pago_monto,pago_moneda,pago_creado_en,pago_actualizado_en,pago_qr_id,pago_qr_string,pago_raw)
                VALUES (:c,'bnb',:pid,'created',:m,:mon,:cre,:act,:qid,:qstr,:raw)");
            $stmt->bindParam(':c', $codigo);
            $pid = (string)$qrId;
            $stmt->bindParam(':pid', $pid);
            $montoFmt = number_format((float)$monto, MONEDA_DECIMALES, '.', '');
            ...
            try{
                $now = date('Y-m-d H:i:s');
                $stmt = $this->conectar()->prepare("INSERT INTO reserva_pago (reserva_codigo,pago_proveedor,pago_payment_id,pago_status,pago_monto,pago_moneda,pago_creado_en,pago_actualizado_en,pago_raw)
                    VALUES (:c,'bnb',:pid,'created',:m,:mon,:cre,:act,:raw)");
                $stmt->bindParam(':c', $codigo);
                $pid = (string)$qrId;
                $stmt->bindParam(':pid', $pid);
                $montoFmt = number_format((float)$monto, MONEDA_DECIMALES, '.', '');
    ...
    private function upsertPagoDesdeBnb($codigo,$paymentId,$status,$monto,$moneda,$raw=null){
        if(!$this->tablaReservaPagoExiste()){
            return false;
        }
        try{
            $now = date('Y-m-d H:i:s');
            $aprobadoEn = null;
            $this->cargarConfigBnbQr();
            $approved = false;
            if(defined('BNB_APPROVED_STATUSES') && is_array(BNB_APPROVED_STATUSES)){
                $approved = in_array(strtolower((string)$status), array_map('strtolower', BNB_APPROVED_STATUSES), true);
            }
            ...
            $sql = "INSERT INTO reserva_pago
                    ...
                    VALUES
                    (:c,'bnb',:pid,:st,:m,:mon,:cre,:act,:apr,:raw)
    ...
    public function procesarEstadoPagoBnbControlador($codigo,$paymentId,$status,$monto,$moneda,$raw=null){
        $codigo = $this->limpiarCadena((string)$codigo);
        if($codigo===''){
            return false;
        }
        if($paymentId===''){
            $paymentId = 'bnb_'.date('YmdHis').'_'.substr(md5($codigo),0,8);
        }
        $this->upsertPagoDesdeBnb($codigo, $paymentId, $status, (float)$monto, (string)$moneda, $raw);
        ...
        if(defined('BNB_AUTO_CONFIRM_RESERVA') && BNB_AUTO_CONFIRM_RESERVA===true && $approved){
            $usuarioAuto = defined('BNB_AUTO_CONFIRM_USUARIO_ID') ? (int)BNB_AUTO_CONFIRM_USUARIO_ID : 1;
            return $this->confirmarReservaDesdePagoOnline($codigo, (float)$monto, $usuarioAuto);
        }
        return true;
    }
    ...
    public function generarPagoBnbQrControlador(){
        ...
        if(!$bnb->configuracionValida()){
            echo "BNB QR no está configurado. Completa config/bnb_qr.php con tu base URL + credenciales.";
            return;
        }

        $moneda = defined('BNB_CURRENCY_ID') ? (string)BNB_CURRENCY_ID : 'BOB';
        $descripcion = 'Reserva '.$reserva['reserva_codigo'].' - '.$reserva['producto_nombre'];

        // Expiración: por simplicidad, 2 días desde hoy (formato YYYY-MM-DD)
        $exp = date('Y-m-d', strtotime('+2 day'));
        $resp = $bnb->getQRWithImageAsync($descripcion, (float)$monto, true, $exp, null);
        if(!is_array($resp) || (($resp['_http_code'] ?? 0) < 200) || (($resp['_http_code'] ?? 0) >= 300)){
            echo "No se pudo generar el QR en BNB (revisa endpoints/credenciales).";
            return;
        }

        // En el portal sandbox, la respuesta útil suele venir dentro de Mensaje como JSON string.
        ...
        if(!is_array($payload)){
            // fallback: si el endpoint devuelve el JSON directo
            $payload = $resp;
        }

        $qrId = (string)($payload['qrCode'] ?? ($payload['qrId'] ?? ($payload['id'] ?? '')));
        $qrImage = (string)($payload['qrImage'] ?? ($payload['qr_image'] ?? ''));
        ...
        $qrString = '';
        if($qrImage!==''){
            // Guardamos como Data URI para poder renderizar directo.
            $qrString = 'data:image/png;base64,'.$qrImage;
        }

        $this->registrarReservaPagoCreadoBnb(
            (string)$reserva['reserva_codigo'],
            $qrId,
            $qrString,
            (float)$monto,
